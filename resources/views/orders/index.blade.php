            ...
            <table class="table">
                ...
                @foreach($orders as $order)
                    <tr>
                        ...
                        <td><a href="{{url('/details/'.$order->id)}}" class="text-white bg-dark p-2 mt-2">نمایش جزییات</a></td>
                        <td scope="2">
                            <form action="{{url('/orders/update/'.$order->id)}}" method="post">
                                @csrf
                                @method('PUT')
                                    <div class="d-flex d-inline-flex">
                                        <select name="status" class="form-control status" data-id="{{ $order->status_id}}">
